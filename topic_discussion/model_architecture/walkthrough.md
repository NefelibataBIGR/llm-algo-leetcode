...
如果把它写得更像知识组织者，它还应该补上三件事：

- 为什么这条线重要：因为结构决定了后续训练、推理和显存判断的共同参照系。
- 技术怎么演化：从基础 norm / attention / MLP，到 MoE / sparsity，再到工程里的局部 trick，是同一条结构线上不同层级的取舍。
- 读完能做什么：能画出 block 图、能解释组件位置、能把教科书结构对应到真实源码。

## 当前定位

这一页现在更适合当“连续讲述总线”：

- 它负责把 `01-08` 重新串成一条故事线
- 它也负责把 `02_tokenization_embedding.md`、`03_norm_evolution.md`、`04_attention_evolution.md`、`05_rope_position_encoding.md` 串起来读
- 如果你想先看某个模块的演进，不必先读完整页，直接进单页即可
...
2. 结构演化是怎么发生的
- 从基础的 norm / attention / MLP，到 MoE / sparsity，再到工程里的局部 trick，都是在同一条结构线上做取舍。
...
如果要继续加厚这一页，最值得补的是两种对照：

- 教科书 block 和真实实现的对照
- Dense MLP 和 MoE 替换位置的对照
...
如果某个专题只有文件列表、没有因果叙事，那它还没有真正成为专题。
