...
3. `75` 把显存上限、最低吞吐和质量下限写成预算，筛选可行方案，并做必要的压力或敏感性实验。
4. `74` 用 profiling 检查显存变化是否真的对应 activation、搬运、重算或 kernel 瓶颈，而不是只看一个峰值数字。
5. 每一步都保存配置、环境、候选结果和 `accept / tune / reject` 决策；只有同一 workload 下的对照才可以进入最终表格。

当前训练侧结果说明 checkpoint 能在高压力序列长度下节省约 332 MB 显存，但会牺牲吞吐；因此 `76` 的结论应写成“在预算约束下可行”，而不是“普遍更优”。
...
- [76 Activation / Checkpoint / Offload Benchmark](../../02_PyTorch_Algorithms/76_Activation_Checkpoint_Offload_Benchmark.ipynb)
- [75 Memory Budget Compression Project](../../02_PyTorch_Algorithms/75_Memory_Budget_Compression_Project.ipynb)
- [74 Profiling Driven End-to-End Optimization](../../02_PyTorch_Algorithms/74_Profiling_Driven_End_to_End_Optimization.ipynb)
