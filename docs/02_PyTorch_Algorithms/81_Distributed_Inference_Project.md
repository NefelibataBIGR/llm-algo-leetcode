...
这一节对应的真实项目问题不是“分布式推理能不能搭起来”，而是“在既定 workload、路由约束和通信成本下，当前 serving 方案是否值得迁移到分布式部署”。分布式推理项目最务实的切入点不是先搭真实多卡环境，而是先做单卡逻辑验证，在有限资源下把 workload、路由、逻辑成本和迁移判断先跑通，再决定是否值得进入真实多卡 serving。

本节的核心矛盾是扩展收益、路由均衡与通信代价之间的权衡：分布式推理可以提升总吞吐、分散请求压力，也可能因为 route 失衡、跨副本通信和 makespan 拉长而把收益抵消掉。做完这一节，你应该能输出一份 baseline vs distributed serving candidate 的迁移结论，而不只是跑一组逻辑模拟结果。

因此，这一页把分布式推理收成一个最小项目交付入口：先定义 workload 与虚拟集群，再确认 baseline 与迁移口径合法，用统一口径比较路由结果、负载均衡和通信代价，并把结论收成 `accept / tune / reject` 的项目判断。它直接承接 `66 / 70 / 79 / 2.9` 的推理、服务与并行直觉，并继续通向 `80` 的 MoE 专家并行和 `74` 的 profiling 驱动优化。
