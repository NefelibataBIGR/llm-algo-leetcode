...
这一节对应的真实项目问题不是“量化后 bit 数降了多少”，而是“在既定 workload、误差约束和部署环境下，这个量化方案是否值得上线”。真实工程里，读者真正要判断的不是单独的 VRAM 降幅，而是 baseline 与 quantized 方案在 latency、throughput、误差和运行后端约束固定之后，是否还能支撑可解释的部署选型结论。

本节的核心矛盾是压缩收益与部署代价之间的权衡：量化可以降低显存占用、释放 batch 或上下文空间，也可能带来 kernel 支持限制、反量化开销和输出误差。做完这一节，你应该能输出一份 baseline vs quantized 的部署选型结论，而不只是列出一张不同 bit 数的 benchmark 表。

因此，这一页把量化推理与部署收成一个最小项目交付入口：先定义部署目标和量化对象，再固定 baseline 与量化方案口径，用统一口径比较速度、显存和误差，并把结论收成是否值得部署的项目报告。它直接承接 `25 / 26 / 66` 和 `P1:21` 的量化与基础推理直觉，并继续通向 `74` 的 profiling 优化和 `70` 的 serving 调度基准。
