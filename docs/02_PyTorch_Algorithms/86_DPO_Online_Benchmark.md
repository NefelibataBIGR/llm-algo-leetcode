...
这一节对应的真实项目问题不是“在线 DPO 胜率能不能涨”，而是“在既定反馈流、更新窗口和安全阈值下，这套在线更新方案是否值得上线”。真实工程里，读者真正要判断的不是单独的 win rate 增量，而是 baseline 与 online candidate 在更新时延、稳定性和安全边界固定之后，是否还能支撑可解释的对齐决策结论。

本节的核心矛盾是在线收益、更新成本与安全风险之间的权衡：在线 DPO 可以更快吸收反馈、提升偏好胜率，也可能因为反馈流质量不足、更新频率过高或安全阈值不稳而把收益抵消掉。做完这一节，你应该能输出一份 baseline vs online candidate 的对齐基准结论，而不只是记录一组 win rate 变化。

因此，这一页把在线 DPO 收成一个最小项目交付入口：先定义在线 benchmark 目标，再确认 baseline 与安全口径稳定，用统一口径比较胜率、更新成本和安全边界，并把结论收成 `accept / tune / reject` 的项目判断。它直接承接 `15 / 16 / 50 / 51 / 84 / 85` 的后训练对齐直觉，并继续回挂到对齐专题的项目决策闭环。
