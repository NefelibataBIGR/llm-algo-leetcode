...
这一节对应的真实项目问题不是“数据能不能读进训练循环”，而是“在既定模板、样本和评测口径下，这批 SFT 数据是否值得进入正式训练”。真实工程里，很多微调失败并不是模型不会训，而是样本空回答、模板字段缺失、重复样本过多，或者评测样例和训练目标根本不一致。

本节的核心矛盾是数据质量、训练成本与交付节奏之间的权衡：数据审得越细，前期成本越高；但如果带着脏样本直接进训练，后面 loss、评测和部署结论都会失真。做完这一节，你应该能输出一份数据是否可训练、是否需要回修模板、是否需要补评测样例的项目结论，而不只是统计几行样本数。

因此，这一页把 SFT 数据质量收成一个最小项目交付入口：训练前做样本审计、模板审计和评测覆盖检查，训练前就把 baseline 数据口径、风险项和后续动作收成 `accept / tune / reject` 的项目结论。它直接承接 `09 / 13 / 30 / 32` 的训练闭环与数据工程内容，并继续通向 `62` 的指令微调项目和 `65` 的低资源微调选型。
