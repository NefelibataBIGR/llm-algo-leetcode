...
硬件选型真正难的地方，不是把峰值算力和显存容量抄成一张表，而是判断一套芯片方案在软件栈成熟度、迁移成本、运维稳定性和生态兼容性上能不能真的落地。纸面规格接近，不代表实际部署成本也接近。

这一页在整个教程的纵向主线里属于 `Part 01` 的系统选型补充页，不是某条主学习路线的核心机制页，但会给后面的训练、推理和部署判断补一层现实约束。学完这里，后面再看 `26 / 33` 以及 `Part 03 / Part 04` 的实现与系统优化页时，你会更容易把硬件规格、编译适配、通信能力和迁移代价放进同一张决策表；如果这里没学明白，后面很容易只按峰值参数做静态排序，而忽略软件栈成熟度和迁移成本。按专题归类，这一页更接近全教程的系统背景补充，同时和 `通信与并行专题`、`编译与图优化专题` 共享部分选型视角。
