...
这一页在整个教程的纵向主线里属于 `Part 01` 的训练吞吐与精度基础页，优先服务 `监督微调路线` 的训练控制和低显存微调判断。学完这里，后面再看 `13 / 60 / 65` 时，你会更容易读懂 `BF16 / AMP / Tensor Core` 为什么会同时影响训练吞吐、训练稳定性和低显存微调选择；如果这里没学明白，后面很容易只记住“Tensor Core 很快”这个结论，却说不清吞吐收益、数值稳定性和显存取舍为什么会绑在一起。按专题归类，这一页主要属于 `监督微调路线` 的训练前置，也和 `量化与压缩专题`、`Profiling 专题` 共享部分判断视角。
